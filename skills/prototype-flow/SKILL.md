...
用户只说“帮我开始”或不知道该做什么时，先用上述只读入口检查当前目录和已提供的资料，结合本次目标推荐起点。复用已知的项目、材料和范围，不要求用户先学会命令、选择依赖 Skill 或填写完整信息表。缺口会改变目标、范围或业务行为时才问一个关键问题；没有资料时，请用户说明要解决的问题或提供材料位置，不虚构需求。

| 当前依据 | 建议起点 |
| --- | --- |
| 想法、会议记录或零散资料 | 整理为 PRD，保留来源和待决项 |
| 已有 PRD | 检查规则、验收与关键缺口，按本次目标接入需求或进入 Demo |
| 已有 Demo | 核对需求依据和实际产物，接入或继续指定修改 |
| 已有项目或中断任务 | 定位相关模块、任务和已保存成果，核对当前输入后续作 |

开始执行前，用一句话说明本次处理范围、预期产物和结束条件；简单请求可合并到正常进展说明中。已经授权连续完成多个阶段时，阶段切换只同步进展并继续，不逐步索要确认。关键业务缺口只暂停依赖它的部分；用户只要求 PRD 时，以 PRD 交付为本次终点。

用户要求“继续”时，优先沿用当前对话明确的范围，通过 `state` 及相关 `runs/artifacts` 查询恢复上下文，说明已保存成果、剩余工作和实际阻碍。核对任务固定输入与当前 PRD；输入有变化时先判断影响，再按本次授权处理。多个候选且上下文无法定位时才询问继续对象，不自动选择全项目或最近一条无关任务。任务记录不证明 Agent 仍在运行，阶段标签也不代表实施授权；历史任务续作仍遵循 [Demo 与变化](references/demo-and-change.md) 的恢复规则。
...
- **demo-design**：涉及产品 Demo 设计、实现或更新时读取当前 Skill，按本次变化选择适用 route、tier、product_contract 和验证要求；有界规则修改不因涉及业务语义自动扩大为完整重做。首次生成或接入 Demo 时建立[项目共享框架](references/demo-framework.md)，后续固定框架版本与已有完整 Demo，复用导航、组件和业务状态；需求影响公共能力时同任务更新框架和 Demo。这里不扩展其 schema，也不将管理契约渲染进产品 Demo。
...
- 每次 Demo 任务先 `run-start` 固定输入，按输入制作；所属文档及已声明依赖自动作为有效性依据，额外共享规则用 `--documents` 指定。理解需求的输入闭包不等于本次复验范围。用户随后修改 PRD 时不覆盖新正文、不把旧输入 Demo 标为最新。Agent 继续判断未声明的共享规则影响；完成时优先 `demo-finalize` 一次登记产物、关联和任务结果，中断用 `run-finish` 保留实际成果。
...
交付时让用户能回答“完成了什么、在哪里查看、接下来做什么”：提供实际成果入口，说明本次目标是否完成；仍需推进时给出一个具体的下一步及原因，目标已完成时只推荐有价值的可选后续。确有不同选择时最多补两个备选。需要 Agent 后续操作时，附一条可直接复用的自然语言请求，带上已知项目目录、模块或需求、动作和预期成果，不让用户重新填写已知信息。需要用户决定时，直接列出具体问题及影响；需要用户验收时，给出实际 Demo 入口、操作路径和预期结果。

下一步由本次目标、当前成果与阻碍共同决定，不按模块阶段机械推进，也不把“暂无待办”当作目标已完成：

| 当前情况 | 交付中的引导 |
| --- | --- |
| 关键规则仍有冲突或缺口 | 指出影响的需求和需要决定的问题，不默认催促制作 Demo |
| 需求已具备制作条件，目标包含 Demo | 在已有授权内继续制作；尚未授权的后续工作提供具体请求 |
| Demo 已完成适用验证 | 提供本次范围的验收路径，不推定用户已确认 |
| PRD 已变更，Demo 或证据对应旧稿 | 指明差异与受影响范围，建议分析、更新和复验 |
| 工作中断或受阻 | 说明保留的成果、剩余工作、阻碍及恢复方式 |
| 本次目标已经完成 | 明确本次结束；有价值的后续注明可选，无需为串联流程扩展任务 |

这些引导在 Agent 对话中提供，复用现有项目和任务记录；不新增进度账本，不声称工作台已有“建议下一步”或“复制继续指令”功能。

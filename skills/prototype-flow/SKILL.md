...
需求整理拆分完成、PRD 与需求及来源已保存且必要检查通过后，自动运行 `python3 -B <skill-dir>/scripts/flow.py serve <project-dir>` 启动工作台，无需用户再次提出打开请求；通过宿主支持的后台或持久会话保持服务运行，确认返回的 `projectRoot` 是当前项目，使用宿主可用的浏览器能力打开返回的 `url`，并在交付中提供该本机链接。同一项目已有可用服务时复用并刷新页面，不重复启动。用户明确要求不启动时遵从；宿主权限或能力限制导致无法启动或打开时，如实说明实际状态及可用的启动命令或链接。

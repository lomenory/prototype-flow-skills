...
工作台默认进入用户流程，图标侧栏依次为用户流程、PRD 与需求、项目总览；顶部展示项目名称与版本选择。PRD 目录、正文和关联证据铺满工作区；项目总览合并进度、版本历史与资料依据，顶部统计面板展示当前版本摘要及 Demo 数量。
...
卡片依次显示图标与标题、最多两行的 12px 描述、状态与类型标签；需求卡片第三行右侧的复制图标复制名称和稳定编号，不打开抽屉。搜索仅匹配当前版本画布中的节点名称与编号，保留全部节点并高亮匹配项，按 Enter 定位下一个。
...
顶部可切换流程图和 Demo 演示模式，标题栏下方直接展示独立预览源的 Demo。右上角悬浮需求列表可收起，点击单一有效状态的需求直接切换，多状态需求展开后选择；没有有效页面时说明缺口，不猜测路由。Demo 模式搜索当前登记需求及状态。切换模式或暂时离开用户流程会保留已加载 Demo；选中状态表示最近选择的目标，不自动跟随 Demo 内部交互。历史版本使用对应快照资源，不回退到当前稿。
